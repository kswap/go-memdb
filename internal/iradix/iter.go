...
func (i *Iterator) SeekPrefixWatch(prefix []byte) (watch <-chan struct{}) {
	return i.seekPrefix(prefix, true)
}

// SeekPrefix is used to seek the iterator to a given prefix
func (i *Iterator) SeekPrefix(prefix []byte) {
	i.seekPrefix(prefix, false)
}

// seekPrefix positions the iterator at prefix, optionally returning the
// notification channel of the deepest node reached. See Node.get for why the
// non-watching form is kept separate rather than discarding the result.
func (i *Iterator) seekPrefix(prefix []byte, wantWatch bool) <-chan struct{} {
	// Wipe the stack
	i.stack = nil
	n := i.node
	w := &n.watch
