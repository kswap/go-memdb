...
func CopyNode(n *Node) *Node {
	nn := &Node{}
	// Share the notification slot's current channel, if one was ever created.
	nn.watch.v.Store(n.watch.v.Load())
	if n.prefix != nil {
...
func CopyLeaf(l *leafNode) *leafNode {
	...
	ll.watch.v.Store(l.watch.v.Load())
	return ll
}
...
func hasAnyClosedMutateCh(r *Tree) bool {
	for iter := r.root.rawIterator(); iter.Front() != nil; iter.Next() {
		...
		if n.isLeaf() && n.leaf.watch.fired() {
			return true
		}
	}
	return false
	...
	for i := 0; i < 3; i++ {
		...
		for ; snapIter.Front() != nil; snapIter.Next() {
			...
			case "", "a", "ac": // parent nodes all change
				if !n.watch.fired() || n.leaf != nil {
					t.Fatalf("bad")
				}
			case "ab": // unrelated node / leaf sees no change
				if n.watch.fired() || n.leaf.watch.fired() {
					t.Fatalf("bad")
				}
...
// generateUUID replaces the go-uuid dependency the upstream tests used, so that
// vendoring them here adds nothing to the module's requirements.
